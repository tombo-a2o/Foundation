					   ...
					   const struct sockaddr	*remoteAddress)
{
	...
	if (localAddress != NULL) {
		targetPrivate->localAddress = CFAllocatorAllocate(NULL, /*localAddress->sa_len*/ sizeof(struct sockaddr), 0);
		bcopy(localAddress, targetPrivate->localAddress, /*localAddress->sa_len*/ sizeof(struct sockaddr));
	}
	...
	if (remoteAddress != NULL) {
		if (is_same_address(localAddress, remoteAddress)) {
			...
			targetPrivate->remoteAddress = CFAllocatorAllocate(NULL, /*remoteAddress->sa_len*/ sizeof(struct sockaddr), 0);
			bcopy(remoteAddress, targetPrivate->remoteAddress, /*remoteAddress->sa_len*/ sizeof(struct sockaddr));
		}
	}
				     ...
				     const struct sockaddr	*ptrAddress)
{
	...
	targetPrivate->remoteAddress = CFAllocatorAllocate(NULL, /*ptrAddress->sa_len*/ sizeof(struct sockaddr), 0);
	bcopy(ptrAddress, targetPrivate->remoteAddress, /*ptrAddress->sa_len*/ sizeof(struct sockaddr));
			  ...
			  void				*context)
{
	...
	if (address != NULL) {
		switch (errorCode) {
			case kDNSServiceErr_NoError :
				dnsAddress = CFDataCreate(NULL, (void *)address, /*address->sa_len*/ sizeof(struct sockaddr));
				// ... and fall through
			case kDNSServiceErr_NoSuchRecord :
				switch (address->sa_family) {
					case AF_INET :
